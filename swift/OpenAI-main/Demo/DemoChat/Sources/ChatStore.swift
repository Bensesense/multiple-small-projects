...
public final class ChatStore: ObservableObject {
    ...
    private func saveConversations() {
        ...
        do {
            let encoded = try encoder.encode(conversations)
    ...
    private func loadConversations() {
        print("Attempting to load conversations from UserDefaults.")
        if let savedConversations = UserDefaults.standard.data(forKey: "SavedConversations") {
            print("Data found in UserDefaults, attempting to decode.")
            let decoder = JSONDecoder()
            do {
                self.conversations = try decoder.decode([Conversation].self, from: savedConversations)
                print("Conversations successfully loaded from UserDefaults.")
            } catch {
                print("Failed to decode conversations: \(error)")
            }
        } else {
            print("No data found in UserDefaults during load.")
        }
    }
